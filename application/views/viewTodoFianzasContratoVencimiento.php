...
<div class="container">
    <div class="row">
        <div class="col s12">
            <div class="col s12"><h4 class="header">Fianzas / garantías a <?=$diasTolerancia?> días o menos de vencer</h4></div>
        </div>
        <div class="col s12">
            <div style="width: 110%">
                <table class="display dataTable" id="tabla">
                    ...
                    <tr>
                        ...
                        <th>Condiciones</th>
                        <th>Monto (MXN)</th>
                        <th>Vigencia Fianza / garantía</th>
                        <th>Terminado</th>
                        <?php
                        if($permisos['detalle'])
                        {
                    ...
                    foreach ($fianzasContratos as $row)
                    {
                        ...
                        if($vigente)
                            $botonStatus="<a value='".$posiblesEstadosVigentes[$status]["value"]."' class='".$posiblesEstadosVigentes[$status]["class"]."' style='".$posiblesEstadosVigentes[$status]["style"]."' id='botonStatus".$idFianza."' onClick='siguienteStatus($status, $idFianza, $vigente)'>".$posiblesEstadosVigentes[$status]["value"]."</a>";
                        else
                            $botonStatus="<a value='".$posiblesEstadosExpirados[$status]["value"]."' class='".$posiblesEstadosExpirados[$status]["class"]."' style='".$posiblesEstadosExpirados[$status]["style"]."' id='botonStatus".$idFianza."' onClick='siguienteStatus($status, $idFianza, $vigente)'>".$posiblesEstadosExpirados[$status]["value"]."</a>";
                ...
                if($permisos['mostrar'])
                {
                        ...
                        <div class="row">
                            <div class="col offset-s4 s4" align="center">
                                <a class="btn waves-light gradient-45deg-red-pink"
                                   href="<?= base_url("index.php/CrudGeneradorPDF/generarFianzasVencimiento/$diasTolerancia/" . $sinContratoProyecto . "/" . urlencode($fechaInicial) . "/" . urlencode($fechaFinal) . "/$idVigencia/$idPago/$idEmpresaInterna") ?>">Exportar</a>
                            </div>
                        </div>
                        <?php
                    }
                    else
                    {
                        ...
                        <div class="row">
                            <div class="col offset-s4 s4" align="center">
                                <a class="btn waves-light gradient-45deg-red-pink"
                                   href="<?= base_url("index.php/CrudGeneradorPDF/generarFianzasVencimiento/$diasTolerancia/") ?>">Exportar</a>
                            </div>
                        </div>
                        <?php
                    }
